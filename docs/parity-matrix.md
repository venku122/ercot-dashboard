...
PR #9. Retained items remain represented in the React dashboard; intentionally unsupported sources
are documented explicitly rather than left as broken viewer surfaces.
...
| PowerOutage.us outages     | Intentionally disabled; no credential, polling, public chart, or health penalty     | collector startup, source-health filter, and Playwright absence assertion         |
...
| Collector duty cycle       | Retained in Diagnostics for active grid, ancillary, pricing, weather, and EEA loops | `collector-duty-cycle` chart and structured legacy attempts                       |
