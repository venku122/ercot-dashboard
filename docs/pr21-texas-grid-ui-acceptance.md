...
- Long-term load forecast, large-load category/status, and gross retirements retain their exact
  unavailable reasons. The UI does not borrow an adjacent source to fill them.
...
2. Entering the view makes exactly one queryless `GET /api/v1/texas-grid` request. No resource is
   fetched until the user explicitly opens Generator Interconnection or Resource Capacity Trend.
...
5. `grid_resource=gis|resource_capacity_trend` is the canonical optional selection. Initial load and
