...
- Storage, supply/demand, outages, and combined wind/solar publish an epoch plus a DST flag. Epoch is
  ...
  Checkpoint high-water marks are scoped to metric name and normalized tags so a future forecast
  cannot advance the actual-series cursor.
- Supply/demand `data` rows are filtered by the payload's `forecast` flag before actual demand and
  capacity metrics are emitted. Supply/demand and renewable health timestamps use their newest
  actual observation rather than the payload's forecast-aware `lastUpdated` value.
...
the newest core observation timestamp when an adapter provides one. Operations Messages is
explicitly event-driven. An unchanged valid payload resets failures and is reported as success
without resubmitting its rows.
