...
stable dedupe key. Unchanged payloads are successful no-ops; invalid or zero-core payloads submit no
data and report a failed attempt. Five-minute sources begin at staggered offsets, and one adapter
failure cannot terminate the other loops.

Receiver submissions are chunked below 400 KiB so they remain below the default 512 KiB body
limit, including the multi-day generation-outage history.

## SQLite model and migration

`init_db` uses `PRAGMA table_info(metrics)` before adding nullable `metrics.dedupe_key`; historical
rows are not rewritten. A partial unique index applies only when a dedupe key exists. The additive
...
bucketing remain in place. `PRAGMA optimize` runs after metric ingest. The system does not VACUUM
automatically.
...
point limit, selected aggregation, and whether the current bucket is partial.
...
fixed tick sampling, server bucketing, and Chart.js decimation. `IntersectionObserver` mounts only
near-visible charts. The shared cursor publishes at most once per animation frame and redraws only
subscribers currently within the observer margin, while mounted off-screen charts remain idle.
Click-to-pin state follows the user between visible charts.
