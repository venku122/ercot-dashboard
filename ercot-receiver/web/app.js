...
const CHART_CONFIGS = [
	{ type: "header", title: "Real-Time Grid Conditions (60s)" },
	{
		...
		tooltipFooter: (items) => {
			const demandItem = items.find((item) =>
				item.dataset.label?.includes("Demand"),
			);
			const capacityItem = items.find((item) =>
				item.dataset.label?.includes("Capacity"),
			);
	...
	},
	{
		...
		subtitle: "Real-time (60s) · Source: ERCOT",
		description:
			"ERCOT grid frequency should hover near 60 Hz. Deviations reflect system imbalance.",
		yLabel: "Frequency (Hz)",
		yMin: 59.9,
		yMax: 60.1,
	...
	},
	{
		...
		unit: "sec",
		subtitle: "Real-time (60s) · Source: ERCOT",
		description: "Instantaneous time error at the latest sample.",
		yLabel: "Offset (sec)",
		yMin: -15,
		yMax: 0,
		tooltipTimestamp: true,
	...
	},
	{
		...
		series: [
			...
			{
				label: "Solar (GW)",
				metric: "ercot.Real_Time_Data.Total_PVGR_Output",
				secondary: true,
			},
		],
	},
	...
	},
	{
		...
		subtitle: "Real-time (60s) · Source: ERCOT",
		description:
			"Net interchange across ERCOT ties. Positive = Imports into ERCOT; Negative = Exports.",
		...
		series: [
			...
			},
			{
				...
				tag: "ercot_dc_tie:DC_E",
			},
			{
				label: "DC_N (MW)",
				metric: "ercot.DC_Tie_Flows",
				tag: "ercot_dc_tie:DC_N",
			},
			{
				label: "DC_L (MW)",
				metric: "ercot.DC_Tie_Flows",
				tag: "ercot_dc_tie:DC_L",
			},
			{
				label: "DC_R (MW)",
				metric: "ercot.DC_Tie_Flows",
				tag: "ercot_dc_tie:DC_R",
			},
			{
				label: "DC_S (MW)",
				metric: "ercot.DC_Tie_Flows",
				tag: "ercot_dc_tie:DC_S",
			},
		],
	},
	...
	},
	{
		...
		subtitle: "Real-time (60s) · Source: ERCOT Ancillary",
		description:
			"Generation capacity currently offline, excluding quick-start resources (per ERCOT definition).",
	...
	},
	{
		...
		subtitle: "Real-time (60s) · Source: ERCOT Ancillary",
		description:
			"Reserve capacity available from online resources (define which reserve product(s) are included).",
	...
	},
	{
		...
		subtitle: "Updates every 15 minutes · Source: ERCOT SPP",
		description:
			"Time series of settlement point prices. Use spike-aware scaling to preserve readability.",
		yLabel: "Price ($/MWh)",
		ySuggestedMin: -100,
		ySuggestedMax: 500,
		referenceLines: [{ value: 0, color: "rgba(255,255,255,0.35)" }],
		series: [
			{
				label: "HB_HOUSTON",
				metric: "ercot.pricing",
				tag: "ercot_region:HB_HOUSTON",
			},
			{
				label: "HB_NORTH",
				metric: "ercot.pricing",
				tag: "ercot_region:HB_NORTH",
				secondary: true,
			},
			{
				label: "HB_SOUTH",
				metric: "ercot.pricing",
				tag: "ercot_region:HB_SOUTH",
				secondary: true,
			},
			{
				label: "HB_WEST",
				metric: "ercot.pricing",
				tag: "ercot_region:HB_WEST",
				secondary: true,
			},
			{
				label: "HB_HUBAVG",
				metric: "ercot.pricing",
				tag: "ercot_region:HB_HUBAVG",
				secondary: true,
			},
		],
	},
	...
	},
	{
		...
		description:
			"Airport METAR observations near ERCOT load/generation centers.",
		tooltipTimestamp: true,
	...
	},
	{
		...
		subtitle: "Updates ~hourly · Source: METAR",
		description:
			"Observed sustained wind speed. Gusts may not be shown unless explicitly ingested.",
		tooltipTimestamp: true,
		series: METAR_STATIONS.map((code) => ({
			label: code,
	...
	},
	{
		...
		subtitle: "Updates ~hourly · Source: METAR",
		description:
			"Dewpoint is a proxy for humidity and impacts load via comfort cooling demand.",
		tooltipTimestamp: true,
		series: METAR_STATIONS.map((code) => ({
			label: code,
	...
	},
	{
		...
		subtitle: "Updates ~hourly · Source: METAR",
		description:
			"Combined view to correlate weather and grid behavior. Dual-axis must be labeled to avoid misleading scaling.",
		tooltipTimestamp: true,
		series: [
			{
				label: "Temp",
	...
	{ type: "header", title: "System / Meta" },
	{
		...
		description:
			"Share of time the scraper is active. Sustained high values indicate backlog or under-provisioning.",
		yLabel: "Duty Cycle (%)",
		yMin: 0,
		yMax: 100,
		tooltipTimestamp: true,
		series: [
			{
				label: "ercot_realtime",
				metric: "ercot.app.duty_cycle",
				tag: "app:ercot_realtime",
			},
			{
				label: "ercot_ancillary",
				metric: "ercot.app.duty_cycle",
				tag: "app:ercot_ancillary",
				secondary: true,
			},
			{
				label: "ercot_eea",
			...
			},
			{
				label: "ercot_pricing",
				metric: "ercot.app.duty_cycle",
				tag: "app:ercot_pricing",
				secondary: true,
			},
			{
				label: "poweroutages_us",
				metric: "ercot.app.duty_cycle",
				tag: "app:poweroutages_us",
				secondary: true,
			},
			{
				label: "metar",
	...
	},
	{
		...
		description:
			"Emergency Energy Alert (EEA) level indicator. Discrete state; not a continuous value.",
		yLabel: "EEA Level",
		yMin: 0,
		yMax: 3,
		stepSize: 1,
...
function applySectionXAxis(configs) {
	...
	for (const cfg of configs) {
		...
		const hasCanvas =
			!cfg.type || cfg.type === "multi" || cfg.type === undefined;
...
function seriesKey(metric, tags, transform) {
...
function trendModeLabel(mode) {
	...
	if (mode === "weekly") return "Seasonal weekly";
	if (mode === "monthly") return "Seasonal monthly";
	return "Actual";
}

function bucketSecondsForSeasonal(rangeSec, seasonalPeriod) {
	if (seasonalPeriod === 86400) {
		if (rangeSec <= 604800) return 1800;
		if (rangeSec <= 2592000) return 3600;
		return 10800;
	}
	if (seasonalPeriod === 604800) {
		if (rangeSec <= 2592000) return 21600;
		return 86400;
	}
	return 86400;
}

function resolveSeriesTransform(rangeSec, mode) {
	let seasonalPeriod = null;
	if (mode === "daily") {
		seasonalPeriod = 86400;
	} else if (mode === "weekly") {
		seasonalPeriod = 604800;
	} else if (mode === "monthly") {
		seasonalPeriod = 2592000;
	} else if (mode === "auto") {
		if (rangeSec >= 15552000) {
			seasonalPeriod = 2592000;
		} else if (rangeSec >= 2592000) {
			seasonalPeriod = 604800;
		} else if (rangeSec >= 259200) {
			seasonalPeriod = 86400;
		}
	}

	if (!seasonalPeriod) return null;
	return {
		mode,
		label: trendModeLabel(mode),
		seasonal_period: seasonalPeriod,
		bucket_seconds: bucketSecondsForSeasonal(rangeSec, seasonalPeriod),
	};
}
...
function summaryValueForSeries(config, seriesList) {
	...
	if (summary.op === "pair_kpi") {
		...
		const utilization =
			summary.showUtilization && right
				? Math.round((left / right) * 100)
				: null;
		const utilHtml =
			utilization !== null
				? `<span class="kpi-pill">${utilization}% utilized</span>`
				: "";
		const html = `
      <div class="kpi-grid">
        <div class="kpi-block">
...
function setValue(el, config, valueObj, emptyLabel) {
	...
	const numeric =
		typeof valueObj.numericValue === "number"
			? valueObj.numericValue
	...
	if (valueObj.html) {
		...
		if (headerPill) {
			...
			headerPill.textContent =
				typeof utilization === "number" ? `${utilization}% utilized` : "";
			headerPill.style.display =
				typeof utilization === "number" ? "inline-flex" : "none";
		}
		return;
	}
...
function createCard(config) {
	...
	meta.className = "meta";
	meta.textContent = config.metric
...
function valueFromCtx(ctx) {
	if (ctx && ctx.parsed && typeof ctx.parsed.y === "number")
		return ctx.parsed.y;
		...
		ticks: {
			...
			callback: (value) => {
				...
				const formatted = Number.isNaN(numeric)
					? value
					: formatNumber(numeric, config?.format);
				return `${formatted}${unit ? " " + unit : ""}`;
			},
		},
		grid: { color: "rgba(255,255,255,0.035)" },
		title: yTitle
			? { display: true, text: yTitle, color: "#9fb3c8", font: { size: 11 } }
			: undefined,
		min: config.yMin,
		max: config.yMax,
	...
	return seriesList.map((series, idx) => {
		...
		if (series.fillBySign) {
			...
			dataset.borderColor = (ctx) =>
				valueFromCtx(ctx) >= 0 ? "#5de4c7" : "#ff6b6b";
		}
		return dataset;
	});
}
				...
				tooltip: {
					...
					callbacks: {
						...
						label: (ctx) => {
							...
							const labelValue = formatDisplayValue(value, {
								unit: unitLabel,
...
function collectSeriesQueries(
	...
	if (config.metric) {
		...
		const id = seriesKey(config.metric, tags, transform);
		if (!queries.has(id)) {
			queries.set(id, {
				id,
				metric: config.metric,
	...
	if (config.compute) {
		...
		if (op === "sum") {
			for (const series of config.compute.series) {
				collectSeriesQueries(series, queries, labels, false, transform);
			}
			return;
		}
		if (op === "clip_positive") {
			collectSeriesQueries(
				config.compute.source,
				queries,
				labels,
				false,
				transform,
...
function computeSeriesFromMap(config, since, seriesMap, transform) {
	if (config.metric) {
		const key = seriesKey(
			config.metric,
	...
	if (config.compute) {
		const op = config.compute.op;
		if (op === "minus") {
			const left = computeSeriesFromMap(
				config.compute.left,
				since,
				seriesMap,
				transform,
			);
			const right = computeSeriesFromMap(
				config.compute.right,
				since,
				seriesMap,
				transform,
		...
		if (op === "sum") {
			const seriesList = config.compute.series.map((s) =>
				computeSeriesFromMap(s, since, seriesMap, transform),
			);
			return alignSeriesList(seriesList);
		}
		if (op === "clip_positive") {
			const raw = computeSeriesFromMap(
				config.compute.source,
				since,
				seriesMap,
				transform,
...
function computeLatestFromMap(config, latestMap) {
	...
	if (config.compute) {
		...
		if (op === "latest_sum") {
			const points = config.compute.series
				.map((s) => computeLatestFromMap(s, latestMap))
				.filter(Boolean);
			if (!points.length) return null;
			return {
				ts: Math.max(...points.map((p) => p.ts)),
		...
		if (op === "latest_sum_all") {
			const seriesKey = keyFor(
				config.compute.source.metric,
				nowSec() - 86400,
				[],
			);
...
async function renderDashboard() {
	...
	const trendControl = document.getElementById("trend-mode");
	const rangeSec = parseInt(range.value, 10);
	const trendMode = trendControl ? trendControl.value || "actual" : "actual";
	const seriesTransform = resolveSeriesTransform(rangeSec, trendMode);
	const since = nowSec() - rangeSec;
	if (lastRangeSec !== rangeSec || lastTrendMode !== trendMode) {
		seriesCache.clear();
		lastRangeSec = rangeSec;
		lastTrendMode = trendMode;
	}
	...
	for (const config of CHART_CONFIGS) {
		...
		if (
			resolvedConfig.type === "single" ||
			resolvedConfig.type === "single-compute"
		) {
			collectLatestQueries(resolvedConfig, latestQueries, latestLabels);
		} else if (resolvedConfig.type === "table") {
			for (const series of resolvedConfig.series) {
				collectLatestQueries(series, latestQueries, latestLabels);
			}
		} else {
	...
	for (const entry of seriesQueries.values()) {
		const maxPoints = entry.allowDownsample
			? maxPointsForRange(rangeSec)
			: null;
		...
		if (transform) {
			if (transform.bucket_seconds)
				baseQuery.bucket_seconds = transform.bucket_seconds;
			if (transform.seasonal_period)
				baseQuery.seasonal_period = transform.seasonal_period;
		}
		if (maxPoints || transform) {
			const query = maxPoints
				? { ...baseQuery, max_points: maxPoints }
				: baseQuery;
		...
		seriesRequestList.push(query);
		seriesRequestById.set(entry.id, {
			query,
			maxPoints: null,
			since,
			transform: null,
		...
		try {
			if (
				...
				if (resolvedConfig.hideIfZero) {
					const isZero =
						latest && typeof latest.value === "number" && latest.value === 0;
				...
				const scaled = latest
					? applyUnitScale(latest.value, resolvedConfig)
					: null;
				const formatted = latest
					? formatValue(scaled, resolvedConfig.unit, resolvedConfig)
					: null;
			...
			if (resolvedConfig.type === "table" && table) {
				const zoneTooltips = {
					...
					LZ_LCRA:
						"LCRA (Lower Colorado River Authority) load zone (Central Texas region served by LCRA)",
					LZ_SOUTH: "ERCOT South Load Zone",
					LZ_AEN: "Austin Energy load zone",
					HB_HUBAVG:
						"ERCOT Hub Average = simple average of HB_NORTH, HB_SOUTH, HB_HOUSTON, HB_WEST",
				...
				if (resolvedConfig.id === "settlement_top") {
					const maxValue = rows.length
						? Math.max(...rows.map((row) => Math.abs(row.value)))
						: 1;
					rows.forEach((row) => {
						...
						const pct =
							maxValue > 0
								? Math.min(100, (Math.abs(row.value) / maxValue) * 100)
								: 0;
						fill.style.width = `${pct}%`;
						if (row.value < 0) fill.classList.add("negative");
						const value = document.createElement("span");
						value.className = "price-bar-value";
						value.textContent = formatDisplayValue(row.value, {
					...
					rows.forEach((row) => {
						...
						tdLabel.textContent = row.label;
						const tdValue = document.createElement("td");
						tdValue.textContent = formatDisplayValue(row.value, {
							unit: resolvedConfig.unit,
						});
			...
			if (resolvedConfig.type === "multi") {
				seriesList = resolvedConfig.series.map((series) => {
					...
					const meta = series.weatherType
						? weatherMeta(series.weatherType)
						: null;
					const points = computeSeriesFromMap(
						series,
						since,
						seriesMap,
						seriesTransform,
				...
				const meta = resolvedConfig.weatherType
					? weatherMeta(resolvedConfig.weatherType)
					: null;
				const points = computeSeriesFromMap(
					resolvedConfig,
					since,
				...
				seriesList = [
					{
						label: resolvedConfig.seriesLabel || resolvedConfig.title,
						points,
						unit: meta ? meta.unit : resolvedConfig.unit,
					},
				];
			}

			if (resolvedConfig.hideIfEmpty) {
				const hasData = seriesList.some(
					(series) => series.points && series.points.length,
				);
				entry.card.style.display = hasData ? "" : "none";
				if (!hasData) continue;
			}
			...
			if (summary) {
				...
				const lastSeries = seriesList.find(
					(series) => series.points.length > 0,
				);
				const lastPoint = lastSeries?.points[lastSeries.points.length - 1];
				const formatted = lastPoint
					? formatValue(lastPoint.y, resolvedConfig.unit, resolvedConfig)
					: null;
				setValue(value, resolvedConfig, formatted);
			}
