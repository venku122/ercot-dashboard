import {
  collectLatestQueries,
  computeLatestFromMap,
  computeSeriesFromMap,
  seriesKey,
...
const CHART_CONFIGS = [
  ...
  },
  {
    ...
    unit: "sec",
    subtitle: "Real-time (60s) · Source: ERCOT",
    description: "Change in instantaneous time error between 60-second samples.",
...
function setValue(el, config, valueObj, emptyLabel) {
  ...
  const numeric =
    typeof valueObj.numericValue === "number" ? valueObj.numericValue : parseFloat(valueObj.value);
  ...
  return seriesList.map((series, idx) => {
    ...
    if (series.fillBySign) {
      dataset.backgroundColor = (ctx) =>
        valueFromCtx(ctx) >= 0 ? "rgba(93, 228, 199, 0.15)" : "rgba(255, 107, 107, 0.18)";
...
function collectSeriesQueries(config, queries, labels, allowDownsample, transform) {
  if (config.metric) {
  ...
  if (config.compute) {
    ...
    if (op === "minus") {
      collectSeriesQueries(config.compute.left, queries, labels, false, transform);
      collectSeriesQueries(config.compute.right, queries, labels, false, transform);
      return;
    }
    if (op === "sum") {
    ...
    if (op === "clip_positive") {
      collectSeriesQueries(config.compute.source, queries, labels, false, transform);
      return;
    }
    if (op === "diff") {
      collectSeriesQueries(config.compute.source, queries, labels, false, transform);
  ...
  for (const config of CHART_CONFIGS) {
    ...
    if (resolvedConfig.type === "single" || resolvedConfig.type === "single-compute") {
      ...
      if (resolvedConfig.type === "multi") {
        for (const series of resolvedConfig.series) {
          collectSeriesQueries(series, seriesQueries, seriesLabels, true, seriesTransform);
        }
      } else {
    ...
    try {
      ...
      if (resolvedConfig.type === "multi") {
        seriesList = resolvedConfig.series.map((series) => {
          ...
          const points = computeSeriesFromMap(series, seriesMap, seriesTransform).map(
            ([ts, val]) => {
        ...
        const points = computeSeriesFromMap(resolvedConfig, seriesMap, seriesTransform).map(
          ([ts, val]) => ({
            x: ts * 1000,
